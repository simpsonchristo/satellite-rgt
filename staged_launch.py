...
def aim120ToSpace(stage_n = 2):
    ...
    stage_motor_Isp = [[225.7*u.s, 368*u.s, 226*u.s, 368*u.s]]
    ...
    for i in range(len(stage_pay_m)):
        ...
        mi_mf = (stage_pay_m[i] + stage_2_motor1_minert + stage_2_motor1_mprop)/(stage_pay_m[i])
        stage_2_DV[0][0] = (stage_motor_Isp[0][0]*(9.81*u.m/(u.s*u.s))*np.log(mi_mf)).value
        
        mi_mf = (stage_pay_m[i] + stage_2_motor2_minert + stage_2_motor2_mprop)/(stage_pay_m[i])
        stage_2_DV[0][1] = (stage_motor_Isp[0][1]*(9.81*u.m/(u.s*u.s))*np.log(mi_mf)).value
        
        mi_mf = (stage_pay_m[i] + stage_2_motor3_minert + stage_2_motor3_mprop)/(stage_pay_m[i])
        stage_2_DV[0][2] = (stage_motor_Isp[0][2]*(9.81*u.m/(u.s*u.s))*np.log(mi_mf)).value
        
        mi_mf = (stage_pay_m[i] + stage_2_motor4_minert + stage_2_motor4_mprop)/(stage_pay_m[i])
